$(document).ready(function(){
    console.log("setTimeout");
    setTimeout(function(){$('#logo_B').addClass('latido_logo');}, 600);
   
    setTimeout(function(){$('#logo_S').addClass('latido_logo');}, 650);
    setTimeout(function(){$('#logo_T').addClass('latido_logo');}, 700);
    setTimeout(function(){$('#logo_u').addClass('latido_logo');}, 750);
    setTimeout(function(){$('#logo_b').addClass('latido_logo');}, 800);
    setTimeout(function(){$('#logo_e').addClass('latido_logo');}, 850);
    
});

console.log("app.js");

